...
class PerformanceMonitor(
    ...
    fun startMonitoring() {
        if (isMonitoring.compareAndSet(false, true)) {
            ...
            Logger.d(TAG, "Performance monitoring started for tier: ${tierConfig.name}")
    ...
    fun stopMonitoring() {
        if (isMonitoring.compareAndSet(true, false)) {
            ...
            Logger.d(TAG, "Performance monitoring stopped")
        }
    }
    ...
    private fun collectMetrics() {
        ...
        Logger.d(TAG, "Performance metrics: frameDropRate=${metrics.frameDropRate}, " +
    ...
    private fun performDegradation(metrics: PerformanceMetrics) {
        Logger.w(TAG, "Performing performance degradation due to poor metrics: $metrics")
        ...
        if (currentBitrateReduction < adaptiveStrategies.bitrateReduction.maxReduction) {
            ...
            Logger.i(TAG, "Reduced bitrate by ${(newReduction * 100).toInt()}%")
            return
        }
        ...
        if (currentResolutionIndex < adaptiveStrategies.resolutionDegradation.steps.size - 1) {
            ...
            Logger.i(TAG, "Degraded resolution to ${newResolution.width}x${newResolution.height}")
            return
        }
        ...
        if (currentFpsIndex < adaptiveStrategies.fpsDegradation.steps.size - 1) {
            ...
            Logger.i(TAG, "Degraded FPS to $newFps")
            return
        }
        
        Logger.w(TAG, "All degradation strategies exhausted, performance may be poor")
    }
    ...
    private fun performRecovery(metrics: PerformanceMetrics) {
        Logger.i(TAG, "Performing performance recovery due to stable metrics: $metrics")
        ...
        if (currentFpsIndex > 0) {
            ...
            Logger.i(TAG, "Recovered FPS to $newFps")
            return
        }
        ...
        if (currentResolutionIndex > 0) {
            ...
            Logger.i(TAG, "Recovered resolution to ${newResolution.width}x${newResolution.height}")
            return
        }
        ...
        if (currentBitrateReduction > 0) {
            ...
            Logger.i(TAG, "Recovered bitrate reduction to ${(newReduction * 100).toInt()}%")
        }
    }
    ...
    fun resetAdaptiveState() {
        ...
        Logger.d(TAG, "Adaptive state reset")
    }
}
