...
- (void)stopRecording:(void(^)(SRRecordingResult *result))callback {
    [SRLogger debug:@"SRRecordingManager" message:[NSString stringWithFormat:@"stopRecording called, current status: %ld", (long)self.currentStatus]];
    
    // 如果已经在停止中，忽略重复调用（不返回错误，因为第一次停止可能已经成功）
    if (self.currentStatus == SRRecordingStatusStopping) {
        [SRLogger debug:@"SRRecordingManager" message:@"stopRecording already in progress, ignoring duplicate call"];
        // 不调用 callback，等待第一次停止完成
        return;
    }
    
    // 只有状态不是 RECORDING 且不是 STOPPING 时，才返回错误
    ...
    [[RPScreenRecorder sharedRecorder] stopCaptureWithHandler:^(NSError * _Nullable error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error) {
                [SRLogger error:@"SRRecordingManager" message:@"Stop capture error" error:error];
                SRRecordingResult *result = [SRRecordingResult failureWithErrorCode:SRErrorStartFailed errorMessage:error.localizedDescription];
                [self cleanup];
                if (callback) { callback(result); }
                return;
            }
            [self finishWritingWithCallback:^(SRRecordingResult *result) {
                // 录制过短处理（<1s）- finishWritingWithCallback 已经检查并返回失败结果
                if (!result.isSuccess && result.errorCode == SRErrorDurationTooShort) {
                    // 删除文件（如果存在）
                ...
                // 保存到相册并返回文件名、assetLocalId 和 md5
                NSString *fileName = self.outputURL.lastPathComponent;
                [self saveOutputToPhotoLibrary:self.outputURL completion:^(BOOL success, NSString * _Nullable localId, NSError * _Nullable err) {
                    if (success && self.outputURL) { [[NSFileManager defaultManager] removeItemAtURL:self.outputURL error:nil]; }

                    // 先构造基础结果
                    SRRecordingResult *finalResult = [[SRRecordingResult alloc] init];
                    finalResult.isSuccess = result.isSuccess && success;
                    finalResult.filePath = fileName; // 返回文件名
                    finalResult.fileSize = result.fileSize;
                    finalResult.duration = result.duration;
                    finalResult.assetLocalId = localId;
                    finalResult.errorCode = finalResult.isSuccess ? 0 : SRErrorSaveToPhotosFailed;
                    finalResult.errorMessage = finalResult.isSuccess ? nil : (err.localizedDescription ?: [SRErrorCodes messageFor:SRErrorSaveToPhotosFailed]);

                    // 若保存成功，则计算相册资源 MD5
                    if (finalResult.isSuccess && localId.length > 0) {
                        [self computeMd5ForAssetLocalId:localId completion:^(NSString * _Nullable md5) {
                            finalResult.fileMd5 = md5;
...
- (void)finishWritingWithCallback:(void(^)(SRRecordingResult *result))callback error:(NSError *)error {
    ...
    if (self.assetWriter && self.assetWriter.status == AVAssetWriterStatusWriting && self.hasStartedWriterSession) {
        if (self.videoInput) {
            ...
            @catch (NSException *e) { [SRLogger warn:@"SRRecordingManager" message:[NSString stringWithFormat:@"markAsFinished(video) skipped: %@", e.reason]]; }
        }
        if (self.audioInput) {
            @try { [self.audioInput markAsFinished]; }
            @catch (NSException *e) { [SRLogger warn:@"SRRecordingManager" message:[NSString stringWithFormat:@"markAsFinished(audio) skipped: %@", e.reason]]; }
        }
    } else {
    ...
    [self.assetWriter finishWritingWithCompletionHandler:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.assetWriter.status == AVAssetWriterStatusCompleted) {
                ...
                // 检查时长，如果 < 1s，直接返回失败结果，不调用 onRecordingStopped:
                // 让 stopRecording 统一处理回调
                if (duration < 1.0f) {
                    SRRecordingResult *shortResult = [SRRecordingResult failureWithErrorCode:SRErrorDurationTooShort 
                                                                                errorMessage:[SRErrorCodes messageFor:SRErrorDurationTooShort]];
                    shortResult.duration = duration;
                    shortResult.fileSize = fileSize;
                    if (callback) {
                        callback(shortResult);
                    }
                    [self cleanup];
                    return;
                }
                ...
                // 不在这里调用 onRecordingStopped:，让 stopRecording 统一处理所有回调
                // if (self.recordingCallback) {
...
- (void)saveOutputToPhotoLibrary:(NSURL *)fileURL
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if (completion) completion(success, createdLocalId, error);
    }];
}
...
- (void)computeMd5ForAssetLocalId:(NSString *)localId completion:(void(^)(NSString * _Nullable md5))completion {
    PHAsset *asset = [[PHAsset fetchAssetsWithLocalIdentifiers:@[localId] options:nil] firstObject];
    if (!asset) { if (completion) completion(nil); return; }
    NSArray<PHAssetResource *> *resources = [PHAssetResource assetResourcesForAsset:asset];
    PHAssetResource *videoRes = nil;
    for (PHAssetResource *res in resources) {
        if (res.type == PHAssetResourceTypeVideo || res.type == PHAssetResourceTypePairedVideo) { videoRes = res; break; }
    }
    if (!videoRes) { if (completion) completion(nil); return; }

    CC_MD5_CTX ctx; CC_MD5_Init(&ctx);
    PHAssetResourceRequestOptions *opts = [[PHAssetResourceRequestOptions alloc] init];
    opts.networkAccessAllowed = NO;
    [[PHAssetResourceManager defaultManager] requestDataForAssetResource:videoRes options:opts dataReceivedHandler:^(NSData * _Nonnull data) {
        CC_MD5_Update(&ctx, data.bytes, (CC_LONG)data.length);
    } completionHandler:^(NSError * _Nullable error) {
        if (error) { if (completion) completion(nil); return; }
