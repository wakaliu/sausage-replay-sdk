...
- (IBAction)initButtonTapped:(UIButton *)sender {
    ...
    // 使用新的初始化接口，enableDebugLog 默认为 YES（开启日志）
    BOOL success = [SausageReplayIOSSDK initializeWithPreset:selectedPreset enableDebugLog:YES];
    if (success) {
    ...
    [SRRecordingManager stopRecording:^(SRRecordingResult *result) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (result.isSuccess) {
                ...
                if (result.filePath) {
                    [self addLog:[NSString stringWithFormat:@"文件名: %@", result.filePath]];
                }
                if (result.fileMd5) {
                    [self addLog:[NSString stringWithFormat:@"文件MD5: %@", result.fileMd5]];
                }
                if (result.assetLocalId) {
                    [self addLog:[NSString stringWithFormat:@"相册资源ID: %@", result.assetLocalId]];
                }
            } else {
                [self addLog:[NSString stringWithFormat:@"❌ 录制停止失败 (错误码: %ld): %@", 
                              (long)result.errorCode, result.errorMessage ?: @"未知错误"]];
            }
...
- (void)onRecordingStopped:(SRRecordingResult *)result {
    ...
    if (result.isSuccess) {
        ...
        [self addLog:[NSString stringWithFormat:@"❌ 录制失败 (错误码: %ld): %@", 
                      (long)result.errorCode, result.errorMessage ?: @"未知错误"]];
    }
}
